import { localizedField, lookupProduct, normalizeBarcode, productLabel, resolveCountry, resolveLanguage } from '../../src/services/productLookup';
...
describe('product lookup', () => {
  ...
  it('prefers the selected language, then English, then the original value', () => {
    const product = { product_name: 'Nom français', product_name_de: 'Deutscher Name', product_name_en: 'English name' };
  ...
  it('uses a custom label without an online request', async () => {
    ...
    await expect(lookupProduct('7612345678901', [{ barcode: '7612345678901', label: 'Moon Milk' }])).resolves.toMatchObject({ barcode: '7612345678901', label: 'Moon Milk', exactLabel: 'Moon Milk', source: 'custom' });
    expect(fetchSpy).not.toHaveBeenCalled();
  });

  it('uses the product family API and returns a localized generic household label', async () => {
    const fetchSpy = jest.spyOn(global, 'fetch').mockResolvedValue({ ok: true, status: 200, json: async () => ({ product: { product_name: 'Papier toilette douceur', product_name_de: 'Hakle Natürlich Sanft', generic_name_de: 'Toilettenpapier', quantity: '10 Rollen', brands: 'Hakle', product_type: 'product' } }) } as Response);
    await expect(lookupProduct('7612345678901', [], { language: 'de', labelStyle: 'generic' })).resolves.toMatchObject({ label: 'Toilettenpapier', exactLabel: 'Hakle Natürlich Sanft, 10 Rollen', genericLabel: 'Toilettenpapier', productType: 'product', source: 'open-products-family' });
