import { APP_LANGUAGES, translate } from '../../src/i18n';

describe('app translations', () => {
  it('provides every requested language and interpolates localized messages', () => {
    expect(APP_LANGUAGES.map(({ value }) => value)).toEqual(['en', 'de', 'fr', 'it', 'pt', 'pt-BR']);
    ...
    expect(translate('pt-BR', 'languageSaved', { language: 'Português (Brasil)' })).toContain('Português (Brasil)');
  });
});
